    ...
    try:
        ...
        for table_name in tables:
            try:
                ...
                result = duckdb_conn.query(f"SELECT * FROM {table_name} LIMIT 0").df()
...
Do NOT provide any other text. Just the SQL query."""
    ...
    query_text = query_planning.get("answer", "").strip() if isinstance(query_planning, dict) else ""
    ...
    if query_text and len(query_text) > 10:
        try:
            ...
            result = duckdb_conn.query(query_text).df()
...
def _route_question(prompt: str, tools: dict, km: KnowledgeManager, duckdb_conn, faqs: list = None) -> tuple:
    ...
    if matching_faq:
        ...
        try:
            ...
            result_df = duckdb_conn.query(matching_faq["sql"]).df()
            if len(result_df) > 0:
...
```"""
        ...
        sql_text = sql_response.get("answer", "").strip()
...
{answer}"""
            ...
            # If SQL failed, suggest FAQs
            st.warning("⚠️ That question is too specific. Try one of the recommended FAQs below:")
            
            if faqs:
                st.subheader("📋 Recommended Questions (These Always Work):")
                for i, faq in enumerate(faqs[:5], 1):
                    st.markdown(f"{i}. **{faq['question']}**")
                    st.caption(faq['description'])
            
            return "💡 **Tip:** Use one of the recommended FAQ questions above, or rephrase your question to be more specific.", charts
...
if st.session_state.data is not None:
    ...
    with tab1:
        ...
        # Display recommended FAQs (prevents hallucination by guiding users)
        if st.session_state.recommended_faqs:
            with st.expander("📋 **Recommended Questions (Always Work!)**", expanded=True):
                    ...
                    with cols[i % 2]:
                        if st.button(faq['question'], key=f"faq_{i}"):
                            ...
                            with st.chat_message("assistant"):
                                with st.spinner("📊 Executing pre-validated query..."):
                                    try:
                                        response, charts = _route_question(prompt, st.session_state.tools, km, st.session_state.duckdb, st.session_state.recommended_faqs)
        ...
        # Chat input
        if prompt := st.chat_input("Ask about ARPU, churn, technology, demographics... or use FAQ above"):
            st.session_state.chat_history.append({"role": "user", "content": prompt})
            ...
            with st.chat_message("assistant"):
                with st.spinner("🤖 Analyzing question..."):
                    try:
                        # Use tools to gather data - PASS FAQs to prevent hallucination
                        tools = st.session_state.tools
                        
                        # Route question to appropriate tool with FAQs
                        response, charts = _route_question(prompt, tools, km, st.session_state.duckdb, st.session_state.recommended_faqs)
